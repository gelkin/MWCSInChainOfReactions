...
public class EvolveHelper
{
    ...
    public static EvolutionState initialize(ParameterDatabase parameters, int randomSeedOffset, Output output)
    {
        ...
        breedthreads = EvolveHelper.determineThreads(output, parameters, new Parameter(P_BREEDTHREADS));
        evalthreads = EvolveHelper.determineThreads(output, parameters, new Parameter(P_EVALTHREADS));
        ...
        for (x=0;x<random.length;x++)
        {
            ...
            random[x] = EvolveHelper.primeGenerator(new MersenneTwisterFast(seeds[x]));    // we prime the generator to be more sure of randomness.
